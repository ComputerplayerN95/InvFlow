...
router = APIRouter(prefix="/api/categories", tags=["商品类型"])
...
def create_category(data: CategoryCreate, db: Session = Depends(get_db)):
    if db.query(Category).filter(Category.CategoryID == data.CategoryID).first():
        raise HTTPException(status_code=400, detail="类型编号已存在")
