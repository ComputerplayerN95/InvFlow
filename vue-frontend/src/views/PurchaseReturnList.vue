    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span style="font-size:16px;font-weight:600">采购退货管理</span>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit?'编辑退货单':'新增退货单'" width="900px" destroy-on-close>
      <el-form :model="form" label-width="100px" :rules="formRules" ref="formRef">
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="原采购单" prop="PurchaseID">
              <el-select v-model="form.PurchaseID" filterable placeholder="选择采购单" style="width:100%" :disabled="isEdit" @change="onPurchaseChange">
                <el-option v-for="po in purchaseOrders" :key="po.PurchaseID" :label="`${po.PurchaseID} - ${po.SupplierName}`" :value="po.PurchaseID" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
    ...
    <el-drawer v-model="drawerVisible" title="退货单详情" size="600px">
      <template v-if="currentOrder">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="原采购单">{{ currentOrder.PurchaseID }}</el-descriptions-item>
...
import { purchaseReturnApi, purchaseApi, supplierApi, warehouseApi, productApi } from '../api'
...
const form = ref({ ReturnID: '', PurchaseID: '', SupplierID: '', WarehouseID: '', ReturnDate: '', Operator: '系统', Remark: '' })
const details = ref([])
...
const columns = [
  ...
  { prop: 'PurchaseID', label: '原采购单', width: 140 },
...
const formRules = {
  ...
  PurchaseID: [{ required: true, message: '请选择原采购单', trigger: 'change' }],
}
...
const loadRefs = async () => {
  const [p, s, w, pr] = await Promise.all([
    purchaseApi.list(), supplierApi.list(), warehouseApi.list(), productApi.list()
  ])
  purchaseOrders.value = Array.isArray(p) ? p : (p.data || [])
...
const onPurchaseChange = async (purchaseId) => {
  if (!purchaseId) return
  try {
    const po = await purchaseApi.get(purchaseId)
    form.value.SupplierID = po.SupplierID
    form.value.WarehouseID = po.WarehouseID
    details.value = (po.Details || []).map(d => {
...
const openCreate = async () => {
  isEdit.value = false
  form.value = { ReturnID: genOrderId('PR'), PurchaseID: '', SupplierID: '', WarehouseID: '', ReturnDate: new Date().toISOString().slice(0,19), Operator: '系统', Remark: '' }
  details.value = []
  await loadRefs()
...
const openEdit = async (row) => {
  isEdit.value = true
  form.value = {
    ReturnID: row.ReturnID, PurchaseID: row.PurchaseID,
    SupplierID: row.SupplierID, WarehouseID: row.WarehouseID,
    ReturnDate: row.ReturnDate, Operator: row.Operator, Remark: row.Remark
  }
  await loadRefs()
  await onPurchaseChange(row.PurchaseID)
  const res = await purchaseReturnApi.get(row.ReturnID)
  const order = res
  if (order.Details) {
    order.Details.forEach(d => {
      const idx = details.value.findIndex(x => x.PurchaseDetailID === d.PurchaseDetailID)
      if (idx >= 0) {
...
const handleSave = async () => {
  formRef.value?.validate(async (valid) => {
    if (!valid) return
    if (details.value.length === 0) { ElMessage.warning('请选择原采购单'); return }
    ...
    try {
      const d = validDetails.map((item, idx) => ({
        ReturnDetailID: item.ReturnDetailID || genDetailId('PRD', form.value.ReturnID, idx + 1),
      ...
      if (isEdit.value) {
        await purchaseReturnApi.update(form.value.ReturnID, { ...form.value, Details: d })
      } else {
        await purchaseReturnApi.create({ ...form.value, Details: d })
      }
...
const handleExecute = async (row) => {
  await ElMessageBox.confirm(`确定执行退货 "${row.ReturnID}"？退货后将扣减库存。`, '执行确认', { type: 'success' })
  await purchaseReturnApi.execute(row.ReturnID)
  ElMessage.success('退货执行成功，已更新库存')
  loadData()
}

const handleRollback = async (row) => {
  await ElMessageBox.confirm(`确定回退退货 "${row.ReturnID}"？回退将恢复库存。`, '回退确认', { type: 'warning' })
  await purchaseReturnApi.rollback(row.ReturnID)
